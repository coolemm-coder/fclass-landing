...
Журнал всех правок: **`docs/CHANGELOG.md`** (перенесён из этого файла 2026-07-09).
После изменений — добавлять короткую запись туда, НЕ сюда.

## Деплой — ЕДИНСТВЕННАЯ СХЕМА

Push в main (coolemm-coder/fclass-landing) → GitHub Actions → валидация → curl FTP upload на ISPmanager (178.159.240.15).
- FTP credentials в GitHub Secrets (FTP_SERVER, FTP_USERNAME, FTP_PASSWORD)
- Vercel = ТОЛЬКО staging, не прод
- Ручной lftp — только аварийный fallback
- Формы/вебхуки: https://automation.landingpro.by/webhook/* (emikss.host МЁРТВ с 2026-07-06)
