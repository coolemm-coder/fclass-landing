# CHANGELOG — fclass.by (перенесено из CLAUDE.md 2026-07-09)
...
- `fix(mobile): fill homepage hero without lower seam` (2026-07-27, pending deploy)
  - мобильный фон снова заполняет hero целиком через `cover`, поэтому фотография не заканчивается заметной тёмной полосой;
  - выравнивание по правому краю сохраняет самолёт в кадре;
  - проверено локально на 320 px и 390 px без горизонтального overflow.

- `fix(copy): remove homepage no-subscription claim` (2026-07-27, deployed with `1a69953`)
  - на главной заголовок `Сервисные сборы без абонентской платы` заменён на нейтральный `Прозрачные сервисные сборы` по замечанию клиента;
  - таблица тарифов и пояснение о раздельном показе тарифа поставщика и сервиса First Class не менялись.
...
- `fix(mobile): restore homepage hero fallback and burger overlay` (2026-07-27, deployed)
  ...
  - первый production deploy: `a544f91`; полное мобильное кадрирование опубликовано коммитом `461b19f` после проверки на 320 px и 390 px.


Последние изменения от Codex на 2026-05-15:

- `fix(copy/conversion): clean remaining public claims and lead workflows`
...
- `fix(seo): strengthen business ticket intent`
  ...
  - на обеих страницах убраны спорные карточки `Калькулятор экономии`/`Окупаемость`, заменены на `Калькулятор командировочных` и нейтральные кейсы;
  - обновлены и синхронизированы с видимыми вопросами FAQ JSON-LD, `sitemap.xml` lastmod выставлен на `2026-05-15`; все JSON-LD блоки проверены на валидность.
...
- `fix(conversion): route main contacts through Telegram bot`
  ...
  - для реальной отправки квалифицированных лидов в группу нужно, чтобы логика `@travelangelby_bot` передавала ответы в группу/CRM; шаблоны n8n теперь рассчитаны на env `FCLASS_LEADS_CHAT_ID`, но group chat id всё равно нужно подставить/задать на сервере.
